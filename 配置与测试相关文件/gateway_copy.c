...
char DEVICE_NAME[MAX_LINE_LENGTH];
char PRODUCT_ID[MAX_LINE_LENGTH];
char DEVICE_SECRET[MAX_LINE_LENGTH];
char REGION[MAX_LINE_LENGTH];
...
int main(void)
{
	...
	char DEVICE_NAME_cp[MAX_LINE_LENGTH];
	char PRODUCT_ID_cp[MAX_LINE_LENGTH];
	char DEVICE_SECRET_cp[MAX_LINE_LENGTH];
	char REGION_cp[MAX_LINE_LENGTH];
	...
	Log_i("%f, %f, %f",productData.m_temperature, productData.m_humidity, productData.m_illumination);
...
{
	int i,len, j,rc;
	unsigned char buf[10] = {0x58, 0x59}, recvd = 0;
	...
	for(i = 0; i < TOTAL_PROPERTY_COUNT; i++)
	{
		if(dataTemplates[i].state == cCHANGED)
		{
			...
			buf[3] = i;
			switch(i)sg_subscribe_event_result
			{
				case 0:
				case 1:
				case 2:
			...
			case 4:
				if(checkSum == buf[i])
				{
					...
					switch(recvData[0])
					{
						...
						case 6:
							dataTemplates[recvData[0]].state = eCHANGED;
							dataTemplates[recvData[0]].data_property.type = TYPE_TEMPLATE_FLOAT;
